...
class Connector(AbstractConnector):
    """instantiate a connector for Citation"""

    generated_remote_link_field = "citation_link"
    ...
    def __init__(self, identifier):
        ...
        wrap_str = lambda a: [a]
        get_remote_id = lambda a, *args: self.books_url + str(a)
        self.book_mappings = [
            Mapping("title"),
            Mapping("url"),
        ...
        self.author_mappings = [
            Mapping("name"),
            Mapping("id"),
            Mapping("citationId", remote_field="id", formatter=str),
        ]

    def get_book_data(self, remote_id):
        remote_id = int(get_arxiv_id(remote_id))
        #data_url = f"{self.books_url}{remote_id}"
        data = get_talk(remote_id)
        return data
    ...
    def is_work_data(self, data):
        return True #bool(re.match(r"v", get_arxiv_id(data["id"])))
    ...
    def get_or_create_book(self, remote_id):
        return super().get_or_create_book(self.books_url + str(remote_id))
            
    def parse_search_data(self, data, min_confidence):
        for idx, search_result in enumerate(data.get('entries', [])):
            # build the remote id from the openlibrary key
            key = self.books_url + str(search_result["id"])
